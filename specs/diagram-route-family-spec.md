...
2. **admission PR (`.com`):** populate `HUB_DIAGRAM_ALLOWLIST` with the 21 diagram slugs (zero substrate edits) AND add the `/diagrams` index back to `ROUTE_META` + the committed sitemap. (PR-com-1 deliberately omits the `/diagrams` index entry — advertising/indexing an empty index page is thin-content churn; it goes live, baked + sitemapped, only with content. Pre-PR review finding, cycle 2.)
