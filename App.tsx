        ...
        <div className="hidden md:flex space-x-8">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`text-sm font-medium uppercase tracking-wider hover:text-copper-500 transition-colors ${
                location.pathname === item.path ? 'text-copper-500' : 'text-slate-400'
              }`}
            >
              {item.label}
            </Link>
            ...
            return (
              <React.Fragment key={venture.slug}>
                ...
                <div 
                  ...
                  className={`group relative bg-slate-900/40 border p-8 transition-all duration-300 cursor-pointer flex flex-col h-full ${
                    isCosmocrat 
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-8">
                    ...
                    {/* Status Badge */}
                    <span className={`text-[10px] font-mono px-2 py-1 rounded-sm uppercase tracking-widest ${
                      isCosmocrat 
                  ...
                  <div className="flex-grow">
                    <p className={`text-xs font-mono uppercase tracking-widest mb-1 ${
                      isCosmocrat ? 'text-copper-500' : 'text-slate-500'
                    }`}>
                      {venture.role}
                    </p>
                    <h3 className={`text-2xl font-bold mb-4 ${
                      isCosmocrat || isExpanded ? 'text-white' : 'text-white group-hover:text-copper-400 transition-colors'
                    }`}>
                      {venture.name}
                    </h3>
                  ...
                  <div className="mt-8 flex justify-between items-center border-t border-white/5 pt-4">
                     <span className={`text-xs font-mono uppercase tracking-widest transition-colors ${
                       isExpanded ? 'text-copper-500' : 'text-slate-600 group-hover:text-copper-500'
                     }`}>
                           ...
                           <div className="space-y-6">
                              ...
                              <div>
                                ...
                                <span className="text-slate-400 font-mono text-xs break-all">
                                  danmercede.com/#{expandedItemInRow.slug}
                                </span>
                              </div>
                           </div>
                        </div>
