...
// Typographic dash normalization for the brand dash ban. Long dashes (em U+2014,
// en U+2013, horizontal-bar U+2015) are the canonical "AI tell" and are banned on
// every public surface. Legacy substrate canonicals promoted BEFORE the anti-slop
// gate still carry them; the canonical is immutable (promote-only), so we normalize
// at THIS publish boundary instead of mutating the source. Code-aware (mirrors
// delinkWikiLinks): fenced ```...``` and inline `...` spans are preserved verbatim
// so a CLI flag (`--force`) or a shell range inside a code example is never touched.
// Rule: a long dash between digits becomes a hyphen (numeric range); every other
// long dash becomes a comma (the parenthetical/appositive use, the safe prose default).
const LONG_DASH_CLASS = '—–―';
const DIGIT_RANGE_RE = new RegExp(`(\\d)\\s*[${LONG_DASH_CLASS}]\\s*(\\d)`, 'g');
const LONG_DASH_RE = new RegExp(`\\s*[${LONG_DASH_CLASS}]\\s*`, 'g');

function normalizeDashSegment(seg: string): string {
  return seg.replace(DIGIT_RANGE_RE, '$1-$2').replace(LONG_DASH_RE, ', ');
}
...
export function normalizeDashes(text: string): string {
  ...
  for (const m of text.matchAll(CODE_SPAN_RE)) {
    const idx = m.index ?? 0;
    out += normalizeDashSegment(text.slice(last, idx));
    out += m[0]; // code span preserved verbatim
    last = idx + m[0].length;
  }
  out += normalizeDashSegment(text.slice(last));
  return out;
}
...
export function generateOutput(entries: ThoughtEntry[], diagrams: DiagramEntry[] = []): string {
  ...
  for (const e of entries) {
    lines.push('  {');
    lines.push(`    title: ${JSON.stringify(normalizeDashes(e.title))},`);
    lines.push(`    preview: ${JSON.stringify(normalizeDashes(e.preview))},`);
    ...
    lines.push(`    body: ${JSON.stringify(normalizeDashes(e.body))},`);
    lines.push('  },');
  }
  lines.push('];');
  lines.push('');
  ...
  for (const d of diagrams) {
    lines.push('  {');
    lines.push(`    title: ${JSON.stringify(normalizeDashes(d.title))},`);
    lines.push(`    slug: ${JSON.stringify(d.slug)},`);
    lines.push(`    date: ${JSON.stringify(d.date)},`);
    lines.push(`    alt: ${JSON.stringify(normalizeDashes(d.alt))},`);
    lines.push(`    caption: ${JSON.stringify(normalizeDashes(d.caption))},`);
